...
v20 = [-0.1 0 0]; % initial velocity of particle 2

w10 = [0 0 0]; % initial rotation vector of particle 1
qp10 = [0 w10(3) -w10(2) -w10(1)]; % initial first derivative of first quaternion's derivative
w20 = [0 0 0]; % initial rotation vector of particle 2
qp20 = [0 w20(3) -w20(2) -w20(1)]; % initial first derivative of second quaternion's derivative
...
dt = 0.01; % time step
Niter = 10000; % number of iterations so that Niter*dt = time of the simulation
...
for iter = 1:Niter
    ...
    if rem(iter,100) == 0 % the norm of quaternions does not matter, we renormalise them every 100 iterations to avoid erros
        q1(end,:) = q1(end,:)/qnorm(q1(end,:));
        q2(end,:) = q2(end,:)/qnorm(q2(end,:));
    end
    ...
    if r12 < d12
        ...
        fdis12 = -kd * (v12 + 2*(cross(quat2vec(qprod(qp2(end,:),qconj(q2(end,:)))),s12) - cross(quat2vec(qprod(qp1(end,:),qconj(q1(end,:)))),s21)));
        fdis21 = -fdis12;
    else
        fel12 = [0 0 0];
        fel21 = [0 0 0];
