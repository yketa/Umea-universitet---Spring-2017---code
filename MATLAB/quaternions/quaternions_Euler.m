...
qp1 = [vec2quat(w10)/2]; % first derivative of the quaternion of particle 1 as a function of time (we assume no moment is exerted at t = 0)
qp2 = [vec2quat(w20)/2]; % first derivative of the quaternion of particle 2 as a function of time (we assume no moment is exerted at t = 0)

q1_ = [0 0 0 1] + tan(dt*sqrt(qnorm(qp1(1,:))))*qp1(1,:)/sqrt(qnorm(qp1(1,:)));
q2_ = [0 0 0 1] + tan(dt*sqrt(qnorm(qp1(1,:))))*qp1(1,:)/sqrt(qnorm(qp1(1,:)));

q1 = [0 0 0 1;q1_/sqrt(qnorm(q1_))]; % quaternion of particle 1 as a function of time, initial q is logically the scalar 1
q2 = [0 0 0 1;q2_/sqrt(qnorm(q2_))]; % quaternion of particle 2 as a function of time, initial q is logically the scalar 1

qp1 = [qp1;qprod(q1(end,:),vec2quat(w1(end,:)))/2];
qp2 = [qp2;qprod(q2(end,:),vec2quat(w2(end,:)))/2];
...
for iter = 1:Niter
    ...
    qpn_ = [qpn_;q1(end,:)+tan(dt*sqrt(qnorm(qp1(end,:))))*qp1(end,:)];
